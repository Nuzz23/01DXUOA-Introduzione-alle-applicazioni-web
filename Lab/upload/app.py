...
def nuovo_post():
    if request.method == 'POST':
        ...
        if len(nuovo['text']) < 30 or len(nuovo['text']) > 200:
            ...
            return redirect(url_for("home_page", lista_post=posts, numpost=numpost, user=utente))
        ...
        if nuovo["type"] != '*' and not (3 < len(nuovo['location']) < 40):
            ...
            return redirect(url_for("home_page", lista_post=posts, numpost=numpost))
        ...
        return redirect(url_for("home_page", lista_post=posts, numpost=len(posts), user=utente))
